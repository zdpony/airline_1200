...
public class IntegratedCplexModelV3 {
	...
	public Solution run(List<Aircraft> aircraftList, List<Flight> flightList, List<Airport> airportList, Scenario sce, List<FlightSection> flightSectionList, List<Itinerary> itineraryList, List<FlightSectionItinerary> flightSectionItineraryList, boolean isFractional,  boolean isCancelAllowed){
				...
				if(isFractional){
					...
					for(FlightArc fa:flightArcList){
						...
						if(cplex.getValue(x[fa.id])>1e-5){
							...
							//if this flight is included in a connecting flight pair and this pair is not labeled as feasible, then its brother flight is cancelled
							if(!fa.flight.isStraightened && fa.flight.isIncludedInConnecting && !fa.flight.isConnectionFeasible) {
								fa.flight.brotherFlight.isCancelled = true;
								solution.cancelledFlightList.add(fa.flight.brotherFlight);
							}
