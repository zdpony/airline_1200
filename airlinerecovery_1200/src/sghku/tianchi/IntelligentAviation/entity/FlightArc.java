...
public class FlightArc {
	...
	//计算该arc的成本
	public void calculateCostV2() {
		if (flight.isStraightened) {
			cost += ExcelOperator.getFlightTypeChangeParam(flight.connectingFlightpair.firstFlight.initialAircraftType,
					aircraft.type) * flight.connectingFlightpair.firstFlight.importance;
			...
			cost += Parameter.COST_EARLINESS / 60.0 * earliness * flight.connectingFlightpair.firstFlight.importance;
			cost += Parameter.COST_DELAY / 60.0 * delay * flight.connectingFlightpair.firstFlight.importance;
			cost += Parameter.COST_STRAIGHTEN * (flight.connectingFlightpair.firstFlight.importance
					+ flight.connectingFlightpair.secondFlight.importance);

			if (Parameter.isPassengerCostConsidered) {
				// 如果是联程拉直航班，则只需要考虑联程拉直的乘客对应的delay和cancel cost，普通乘客则不需要考虑（因为在cancel
				// flight和signChange那里会考虑）
				int actualNum = Math.min(aircraft.passengerCapacity,
						flight.connectingFlightpair.firstFlight.connectedPassengerNumber);
				int cancelNum = flight.connectingFlightpair.firstFlight.connectedPassengerNumber - actualNum;
				...
				// 计算中转乘客
				cost += flight.connectingFlightpair.firstFlight.occupiedSeatsByTransferPassenger
						* Parameter.passengerCancelCost;
				cost += flight.connectingFlightpair.secondFlight.occupiedSeatsByTransferPassenger
						* Parameter.passengerCancelCost;

				cancelRelatedCost += flight.connectingFlightpair.firstFlight.occupiedSeatsByTransferPassenger
						* Parameter.passengerCancelCost;
				cancelRelatedCost += flight.connectingFlightpair.secondFlight.occupiedSeatsByTransferPassenger
						* Parameter.passengerCancelCost;

				// 普通旅客的取消成本不在此考虑，而在模型中考虑
				// delayCost += actualNum*ExcelOperator.getPassengerDelayParameter(delay);
				// //record delay cost of connecting pssgr on flight
				// connPssgrCclDueToStraightenCost += cancelNum*Parameter.passengerCancelCost;
				// //record cancel cost due to straighten
			...
			cost += ExcelOperator.getFlightTypeChangeParam(flight.initialAircraftType, aircraft.type)
					* flight.importance;
			...
			// if this flight belongs to a connection flight pair and the corresponding
			// flight is not labeled as feasible, the cost of this arc should also consider
			// the cancellation cost of its brother flight
			if (flight.isIncludedInConnecting && !flight.isConnectionFeasible) {
				cost += Parameter.COST_CANCEL * flight.brotherFlight.importance + flight.brotherFlight.totalConnectingCancellationCost + flight.brotherFlight.totalTransferCancellationCost;
			}
			...
			if (Parameter.isPassengerCostConsidered) {
				...
				if (flight.isIncludedInConnecting) {
					if (!flight.isConnectionFeasible) {
						/*
						 * 首先考虑联程乘客，如果属于联程航班，则代表另一截cancel了， 如果对应第一截catch，第二截cancel，则对应的联程乘客cancel cost
						 * 
						 */
						if (flight.connectingFlightpair.firstFlight.id == flight.id) {
							cost += flight.connectedPassengerNumber * Parameter.passengerCancelCost;
						...
						// 该联程航班依旧有效，计算联程乘客的延误和取消成本
						int cancelConnectingPassenger = Math.max(flight.connectedPassengerNumber - remainingCapacity,
								0);
						int flyConnectingPassenger = flight.connectedPassengerNumber - cancelConnectingPassenger;
				...
				// 考虑中转乘客的延误 -- 假设中转乘客都成功中转
				cost += flight.occupiedSeatsByTransferPassenger * ExcelOperator.getPassengerDelayParameter(delay);
				delayRelatedCost += flight.occupiedSeatsByTransferPassenger
						* ExcelOperator.getPassengerDelayParameter(delay);
